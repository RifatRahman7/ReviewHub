                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {tips.map((tip, index) => (
                        <motion.div
                            ...
                            viewport={{ once: true }}
                            className="bg-white/80 backdrop-blur-sm rounded-xl p-6 shadow-lg hover:shadow-green-500 transition dark:bg-gray-900/70"
                        >
                            <h3 className="text-green-600 text-xl font-semibold mb-2 dark:text-green-400">
                                {tip.title}
                            </h3>
                            <p className="text-gray-700 text-sm dark:text-gray-300">{tip.description}</p>
                        </motion.div>
                    ))}
                </div>
            </div>
